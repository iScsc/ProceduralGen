...
@export var x_dim: float = 1
@export var y_dim: float = 1
@export var z_dim: float = 1
...
func _ready() -> void:
	...
	## Column : Basically a cube with multiple dimensions
	var a := x_dim
	var b := y_dim
	var c := z_dim
	...
	for i in range(2):
		for j in range(2):
			for k in range(2):
				var vert := Vector3(a*(i-.5), b*(j-.5), c*(k-.5))
